...
### 1.1 环境准备
- **Node.js**: 推荐 v16+ (项目中使用 Node.js 运行后端和构建前端)
- **数据库**: MySQL (本项目后端配置了云数据库，无需本地安装 MySQL，但需确保网络能连接公网数据库)
- **包管理器**: 推荐使用 `npm`

### 1.2 后端服务 (Server)
后端基于 Node.js + Express + MySQL。
...
### 1.3 移动端 (Mobile)
移动端基于 Taro + React + NutUI，支持生成 H5、微信小程序等。
...
3. **启动 H5 调试**:
   ...
   npm run dev:h5
   ```
   - 启动后浏览器会自动打开，通常地址为 `http://localhost:10086`。
   - 需在浏览器开发者工具中开启 "Mobile Mode" 获得最佳预览效果。
4. **启动微信小程序调试**:
   ```bash
   npm run dev:weapp
   ```
   - 运行后需打开 "微信开发者工具"，导入 `mobile` 目录进行预览。

### 1.4 管理后台 (Admin)
管理后台基于 Vite + React + Ant Design。
...
3. **启动开发服务**:
   ...
   npm run dev
   ```
   - 启动后控制台会显示访问地址，通常为 `http://localhost:5173`。
